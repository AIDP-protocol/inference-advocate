// Synchronous SHA-256 with no Node or WebCrypto dependency.
//
// Paper: step 9 (ledger hash chaining). The canonical string construction in LedgerStore
// is a protocol decision and stays in core. The digest implementation is a capability, but
...
function pad(bytes: Uint8Array): Uint8Array {
  ...
  // SHA-256 length is a 64-bit big-endian bit count; messages here stay well under 2^32 bits.
  view.setUint32(paddedLen - 4, bitLen >>> 0);
  return out;
}

/** Hex-encoded SHA-256 of a UTF-8 string. Byte-identical to node:crypto createHash('sha256'). */
export function sha256Hex(message: string): string {
  const data = pad(toBytes(message));
  let h0 = 0x6a09e667;
  let h1 = 0xbb67ae85;
  let h2 = 0x3c6ef372;
  let h3 = 0xa54ff53a;
